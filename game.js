...
function applyTier() {
    ...
    try {
        ...
        if (el) {
            ...
            el.textContent = `v=140 · ${IS_ADMIN ? "ADMIN" : "user"} · id=${id || "—"} · q=${adminParam}`;
        }
    } catch (e) {}
}
...
function updateTicketTimer() {
    ...
    const n = Math.abs(state.tickets || 0);
    const lastTwo = n % 100;
    const last = n % 10;
    const attemptWord = last === 1 && lastTwo !== 11
        ? "попытка"
